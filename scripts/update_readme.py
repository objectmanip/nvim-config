...
def make_markdown_table(input_list):
    header: list[str] = list(input_list[0].keys())
    table_string: str = "\n|"+"|".join(header)+"|\n"
...
def get_argument(inbetween: tuple, string, alt_inbetween = None) -> str:
    try:
...
def fix_string(string: str) -> str:
    return string.replace('<', ' \<').replace('>', '\> ').strip().strip('\'\" \n').replace('  ', ' ')
...
def write_themes(readme, columns: int = 2):
    new_readme, readme_after_themes = empty_section(readme, 'Installed themes')
    ...
    new_readme += readme_after_themes
    return new_readme

def write_plugins(readme, columns: int = 2):
    plugin_description_cache = "scripts/plugin_descriptions.json"
